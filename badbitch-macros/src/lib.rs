//! `#[tool(name = "...", description = "...")]` attribute macro.
//!
//! Vendored verbatim from `learn-claude-code-rs`'s `s20_tool_refactor_macros` crate.
//! The generated impls reference `crate::tool::{Tool, ToolContext, input_schema}`, so the
//! consuming crate (`badbitch`) must expose those names from a `tool` module.
//!
//! Two forms:
//!   * stateful: `async fn f(ctx: ToolContext, input: MyInput) -> Result<impl Display>`
//!   * pure:     `async fn f(a: T, b: U) -> impl Display`  (a struct `FInput` is generated)
